...
int main(int argc, char const *argv[]) {
    ...
    int observer_port = 12345; // Port for observer client

    // Create socket for observer client connections
    int observer_socket_fd;
    if ((observer_socket_fd = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0) {
        perror("observer socket failed");
        exit(EXIT_FAILURE);
    }

    // Set up the observer client address structure
    struct sockaddr_in observer_address;
    memset(&observer_address, 0, sizeof(observer_address));
    observer_address.sin_family = AF_INET;
    observer_address.sin_addr.s_addr = htonl(INADDR_ANY);
    observer_address.sin_port = htons(observer_port);

    // Bind the observer socket to the address and port
    if (bind(observer_socket_fd, (struct sockaddr *) &observer_address, sizeof(observer_address)) < 0) {
        perror("observer bind failed");
        exit(EXIT_FAILURE);
    }

    // Listen for observer client connections
    if (listen(observer_socket_fd, max_observer) < 0) {
        perror("observer listen");
        exit(EXIT_FAILURE);
    }

    printf("Observer server is start\n");
